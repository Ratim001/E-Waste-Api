...
[![CI](https://github.com/Ratim001/E-Waste-Api/actions/workflows/ci.yml/badge.svg)](https://github.com/Ratim001/E-Waste-Api/actions/workflows/ci.yml)

REST API prototype that powers authenticated collection, pricing, supplier management, transactions, and analytics for e-waste programs. Built for the ALX capstone requirements with Django 4.2, Django REST Framework, PostgreSQL, JWT auth, Swagger docs, CI, and Render deployment configuration. A live production deployment has not been verified.
...
- **Django apps at the repository root**: `accounts`, `catalog`, `suppliers`, `items`, `transactions`, `analytics`
...
1. **Clone & install**
   ```bash
   git clone https://github.com/Ratim001/E-Waste-Api.git
   cd E-Waste-Api
...
2. **Configure environment**
   ```bash
   cp .env.example .env
   # set DJANGO_SECRET_KEY to a unique generated value; configure DATABASE_URL if using PostgreSQL
   ```
...
2. Use the repository's `render.yaml` as the Blueprint configuration, or configure a Web Service and database manually
3. Verify the database connection and build/start commands in Render; the presence of deployment files alone does not confirm a working deployment
...
This ALX capstone demonstrates backend development for e-waste collection and sales operations. The repository contains the implementation, API tests, a Postman collection, and deployment configuration. CI status is linked above; deployment files are not evidence of a live service.

- Generate a unique `DJANGO_SECRET_KEY` before running outside a disposable local environment. The fallback in settings is for development only.
- Review CORS and host settings before deployment; the current settings allow all CORS origins.
- Local SQLite databases are generated by migrations and are not distributed as project data.
- Seed category prices are demonstration values, not current market quotations.
- Transactions are independent of collected inventory; stock reconciliation is not implemented by the category-and-weight transaction model.
- The existing framework and dependency versions need a supported-version review before production use.

## Sample pricing calculation

For a fictional material category priced at 100 KES/kg, a 2 kg collection in fair condition uses the 0.9 multiplier:

`100 × 2 × 0.9 = 180 KES estimated value`

See [the API tests](tests/test_api.py) for executable authentication, pricing, ownership, transaction, and analytics examples.
